...
const PETAL_GRADIENTS = [
  "from-pink-400/70 to-rose-500/50",
  "from-fuchsia-400/60 to-purple-500/45",
  "from-amber-400/55 to-orange-400/40",
  "from-violet-400/55 to-indigo-500/45",
];
...
const REACTIONS: {
...
}[] = [
  {
    ...
    label: "Люблю",
    color:
      "from-red-500/20 to-pink-500/20 border-red-500/30 hover:border-red-400",
  },
  {
    Icon: Flower2,
    label: "Восхищаюсь",
    color:
      "from-pink-500/20 to-fuchsia-500/20 border-pink-500/30 hover:border-pink-400",
  },
  {
    Icon: Sparkles,
    label: "Вдохновляет",
    color:
      "from-amber-500/20 to-yellow-500/20 border-amber-500/30 hover:border-amber-400",
  },
  {
    Icon: Crown,
    label: "Королева",
...
export default function March8Greeting() {
  ...
  const [petals, setPetals] = useState<
    { id: number; x: number; delay: number; dur: number; g: number }[]
  >([]);
  ...
  const handleReaction = (idx: number) => {
    if (active === idx) {
      ...
      const arr = Array.from({ length: 18 }, (_, k) => ({
        ...
        g: Math.floor(Math.random() * PETAL_GRADIENTS.length),
      }));
      setPetals(arr);
      setShowPetals(true);
      ...
      {showPetals && (
        <div className="fixed inset-0 pointer-events-none z-50" aria-hidden>
          {petals.map((p) => (
            <span
              key={p.id}
              className={`absolute h-2.5 w-2.5 rounded-full bg-gradient-to-br ${PETAL_GRADIENTS[p.g]} shadow-sm shadow-black/20`}
              style={{
                left: p.x + "%",
                top: "-5%",
          ...
          <div className="bg-gradient-to-b from-gray-950 to-gray-950/95 p-8">
            <div className="text-center mb-8">
              <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl bg-gradient-to-r from-purple-500/10 to-amber-500/10 border border-purple-500/20">
                <Sparkles
                  className="h-5 w-5 text-amber-300/90 shrink-0"
                  strokeWidth={1.75}
                  aria-hidden
                />
                <p
              ...
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    disabled={rated}
                    className="transition-transform duration-150 hover:scale-110 disabled:cursor-default p-0.5"
                    aria-label={`Оценить на ${star}`}
                  >
                    <Star
                      className={`h-8 w-8 ${
                        (hoverStar || rating) >= star
                          ? "text-amber-400 fill-amber-400/90 drop-shadow-[0_0_10px_rgba(251,191,36,0.35)]"
                          : "text-gray-600"
                      }`}
                      strokeWidth={1.5}
                    />
              ...
              <div className="flex flex-wrap justify-center gap-3">
                {REACTIONS.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      <Icon
                        className="h-4 w-4 text-gray-200 shrink-0"
                        strokeWidth={1.75}
                      />

...
type ProseWorkPageClientProps = {
  slug: string;
  initialWork: TranslatedWork;
  initialAllProse: TranslatedWork[];
};

export default function ProseWorkPageClient({
  slug,
  initialWork,
  initialAllProse,
}: ProseWorkPageClientProps) {
  const router = useRouter();
  const { language, t } = useLanguage();

  const [work, setWork] = useState<TranslatedWork>(initialWork);
  const [allProse, setAllProse] = useState<TranslatedWork[]>(initialAllProse);

  useEffect(() => {
    setWork(initialWork);
    setAllProse(initialAllProse);
  }, [slug, initialWork, initialAllProse]);
        ...
        <AnimatedSection delay={80}>
          <header className="mb-12 sm:mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 mb-6 sm:mb-8">
              ...
              <Quote
                className="h-3.5 w-3.5 scale-x-[-1] text-amber-400/80"
                strokeWidth={2}
                aria-hidden
              />
            </div>
